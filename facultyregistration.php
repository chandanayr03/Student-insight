 ...
 error_reporting(0);
    ...
    if (isset($_REQUEST['username'])){
		...
		$trn_date = date("Y-m-d H:i:s");
		 ...
		 $query = mysqli_query($con, "SELECT * FROM faculty WHERE username = '".$username. "'");
  if(mysqli_num_rows(  $query) > 0){
    echo "<div class='form'><h3>!!!!!!!!!!! user name already exist please user other.</h3><br/>Click here to try again <a href='facultyregistration.php'>Register</a></div>";
	
}else{
        $query = "INSERT into `faculty` (username, password, email, trn_date) VALUES ('$username', '".md5($password)."', '$email', '$trn_date')";
        $sql = "INSERT into facultyprofile (facultyid) Values ('$username')";
...
<h2>Faculty - Registration</h2>
  <div class="form">
<form name="facultyregistration" action="" method="post">
...
<p>Already registered ? <a href='facultylogin.php'>Login Here</a></p>
</div>
<?php } ?>
          </div>
        </div></div>
